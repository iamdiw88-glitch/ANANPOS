  ...
  return (
    <div className="flex h-screen bg-background overflow-hidden font-sans">
      <aside className="w-64 bg-white border-r border-slate-200/60 flex-col hidden md:flex shrink-0 z-20 shadow-sm relative">
        ...
        <nav className="flex-1 py-5 flex flex-col gap-5 px-3 overflow-y-auto scrollbar-thin">
          {visibleSections.map((section) => (
            <div key={section.title} className="space-y-1.5">
              <div className="px-3 text-[11px] font-bold uppercase tracking-wider text-slate-400">{section.title}</div>
              {section.items.map((item) => {
                ...
                return (
                  <Link
                    key={item.href}
                    href={item.href}
                    className="group flex items-center gap-3 px-3 py-2.5 rounded-lg hover:bg-blue-50 transition-all duration-200 text-sm font-semibold text-slate-600 hover:text-primary relative overflow-hidden"
                  >
                    <div className="absolute left-0 top-0 bottom-0 w-1 bg-primary rounded-r-md opacity-0 group-hover:opacity-100 transition-opacity" />
        ...
        <div className="p-4 border-t border-slate-100 bg-slate-50/50">
          ...
          >
            <button
              type="submit"
              className="flex items-center justify-center gap-2 w-full px-4 py-2 rounded-lg text-sm font-semibold text-red-600 hover:bg-red-50 hover:text-red-700 transition-colors border border-red-100 bg-white shadow-sm"
            >
